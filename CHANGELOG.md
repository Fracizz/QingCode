...
- Markdown 预览支持本地相对图片
- 支持独立文件窗口、单实例打开转发，以及通过 Ctrl+O 从系统对话框打开文件
- 资源管理器收藏夹与搜索、终端工作流联动优化

### Changed

- Windows 安装包默认安装全部语言组件

### Fixed

- 优化收藏夹卡片分区与路径展示，并提升搜索和启动稳定性

## [0.1.11] - 2026-08-07

### Added

- 文件监听与终端会话持久化；设置中提供性能诊断

### Fixed

- 打包版 WebView2 遗失 Ctrl 键事件时，Ctrl+点击仍可跳转到定义
...
- 源代码管理：GIT 地址区域展示多远程；变更/待提交列表支持 Ctrl+A 全选
- 设置：Windows 文本读取模式（自动 / 兼容 / 原生）；关于面板与可复制版本信息
- 顶栏 Git 更改数量角标（CountBadge）；状态栏内存悬停展示主进程 / WebView2 / 终端 / 当前项目终端占用
- 标题栏：无法解码文件的只读预览；大文件预览与侧栏布局交互优化
- 导航：转到定义优先当前文件用法分组；拦截原生 Ctrl+A；多选复制路径

### Changed

- 侧栏布局：终端移至编辑器右侧；标题栏拖动改用 WebView2 `app-region`
- 源代码管理远程展示与工具栏布局整理

### Fixed

- 源码管理首次打开历史为空或仅一条；变更区 ↓ / + / ↑ 操作列垂直对齐（按实测滚动条宽度）
- 更新下载与 UTF-16 编码识别；关于页「复制」按钮换行
- 顶栏与侧栏 Git 角标对齐；标题栏空白区拖动卡顿
...
- 编辑器语法高亮扩展：YAML、TOML、Shell、Rust、Go
- 搜索混排布局与路径快速打开完善
- 源代码管理整页工作台、Git 拉取/冲突提示、批量操作与历史布局优化
- 设置中提供可复制的 AI CLI Skill 文本；应用 CLI IPC
- 终端 Windows Shell 自动选择与降级；安装包缺少 WebView2 时可跳转下载
- 检查更新支持直接下载安装包

### Changed

- 侧栏旁终端默认均分终端与编辑器宽度；切换经典/侧栏布局时保持终端挂载并即时切换
- 资源管理器复制快捷键与项目悬停操作优化；刷新默认可显示 gitignore 条目
- 磁盘同步、SCM 丢弃/文件复制/属性面板等体验增强

### Fixed

- 修复 NSIS 安装包未嵌入最新前端的问题
- 终端面板拖动闪烁与布局切换卡顿
- 编辑器缩进/括号参考线、行号区样式与右键菜单开发模式行为

## [0.1.5] - 2026-07-20

### Added

- 状态栏展示进程内存占用，便于观察编辑器资源开销
- 源代码管理对超长截断路径：悬停约 1 秒显示完整路径

### Changed

- 源代码管理展开未跟踪目录内的文件（类似 VS Code / `git status -uall`），例如显示 `.qingcode/run.json` 而不再只显示整夹
- 差异对比折叠未改动行文案国际化（中文「N 行未更改」）
- 禁用 F5 / Ctrl+R（含 Cmd）触发 WebView 整页刷新，避免误触丢失会话

### Fixed

- 差异对比侧栏无法滚动：修正 MergeView 布局，外层出现滚动条并可浏览全部差异块
- 会话恢复、资源管理器单击打开与 Git 面板在大量变更时的性能/交互问题
- 状态栏 Tooltip 圆角箭头与间距
