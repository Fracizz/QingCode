...
- 面板支持单个/全部暂存、单个/全部取消暂存、只提交已暂存内容，以及向当前分支已配置的 upstream 执行普通 `git push`；写操作完成后统一执行完整状态刷新。
- 提交信息通过系统 Git stdin 传递，正常执行用户身份检查和 hooks；失败时保留输入并显示 Git 输出。
- 不提供隐式暂存、discard、pull/fetch、远程凭据管理或冲突解决。详细安全边界、异常处理和验收场景见 [`docs/git-basic-commit-workflow.md`](docs/git-basic-commit-workflow.md)。
