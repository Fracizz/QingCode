...
双终端 A|B 与终端带|编辑器均可拖；任一侧最大约 **90%**（双终端比例 `0.1–0.9`，终端宽度同带内 10%–90%）。默认比例：单终端|编辑器 **1:1**；双终端+编辑器 **A:B:E ≈ 1:1:1**（终端带:编辑器 = 2:1，双终端内 50%）。顶栏布局菜单只提供三档预设；侧栏布局下，布局按钮左侧另有「双终端」「编辑器」独立图标细调（不进菜单；藏编辑器时菜单无勾选项，便于再点预设恢复）。终端栏仅保留收起 / 关闭全部。所有终端标签栏共用溢出下拉。打开文件 / SCM / 设置只显示编辑器列，**不关闭双终端**。
...
| 项 | 约定 |
|----|------|
| 布局 | 标题栏左侧：应用图标 + 常驻工作区菜单 + 项目 chips 区（`flex-1`）+ 拖拽 spacer（固定 `w-[140px]`）+ QingCode；右侧：侧栏布局时双终端/编辑器快捷开关 + 面板布局切换 + 窗口按钮 |
