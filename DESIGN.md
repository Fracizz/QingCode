...
双终端 A|B 与终端带|编辑器均可拖；任一侧最大约 **90%**（双终端比例 `0.1–0.9`，终端宽度同带内 10%–90%）。默认比例：单终端|编辑器 **1:1**；双终端+编辑器 **A:B:E ≈ 1:1:1**（终端带:编辑器 = 2:1，双终端内 50%）。顶栏布局菜单只提供三档预设；侧栏布局下，布局按钮左侧另有「双终端」「四终端」「编辑器」独立图标细调（不进菜单；藏编辑器时菜单无勾选项，便于再点预设恢复）。「双」与「四」互斥：四为 2×2（TL/TR/BL/BR），十字分割比例持久化于 `qingcode:quad-terminal-ratio`（列/行各 `0.1–0.9`）。终端栏仅保留收起 / 关闭全部。所有终端标签栏共用溢出下拉。打开文件 / SCM / 设置只显示编辑器列，**不关闭双终端 / 四终端**。
