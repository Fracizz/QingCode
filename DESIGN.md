...
- 状态栏使用中性 `bg-deep`，当前项目图标使用 `brand`，避免整条功能蓝背景削弱品牌层级。
...
森林主题采用“深松林底色 + 苔藓灰文字 + 琥珀功能光 + Qing 青品牌轨”。六级背景保持同一青绿色相，但通过明度拉开主编辑区、活动栏、侧栏、浮层、悬停与选中层级；不使用独立纯绿选中块。

| 语义 | 值 | 用途 |
|------|-----|------|
| `bg` / `bg-deep` | `#072a27` / `#041f1d` | 编辑区与活动栏/终端底 |
| `bg-sidebar` / `bg-elevated` | `#0b322e` / `#103a35` | 侧栏与浮层 |
| `bg-hover` / `bg-active` | `#16453e` / `#1b5046` | 悬停与选中 |
| `fg` / `fg-muted` / `fg-dim` | `#d5e2dd` / `#b6c6c0` / `#aabbb5` | 主、次、弱文字 |
| `brand` | `#5ad6be` | QingCode 品牌与 Qing Rail |
| `accent` | `#d3a15b` | 键盘焦点、光标、加载与拖放 |

编辑器和终端共享 `src/lib/materialForestTheme.ts` 的壳层色；语法色保留 Material Forest 的绿、紫、蓝、珊瑚色体系，代码注释调整为 `#789b94` 以满足小字可读性。
...
`pnpm package`（本机 x64 便携+安装包）已自动包含 `icon:sync`。
...
- 禁止 WebView 原生刷新快捷键：`F5`、`Ctrl`/`⌘` + `R`（含 `Shift` 组合）。
- 禁止 WebView 原生开发者工具快捷键：`F12`、`Ctrl+Shift+I/J/C`，以及 macOS 的 `⌘+⌥+I/J/C`。
