...
| 界面辅助字号 | 界面字号 − 1px（默认 12px） | `--ui-font-size-sm` / `.text-ui-sm` |
...
**辅助信息**比正文小一级，并随用户界面字号设置缩放。典型场景：确认框路径块、命令面板中的相对路径、编辑器打开失败时的路径行、设置项描述段落、Toast 次要说明。路径等技术型辅助块可使用带边框容器 + `font-mono text-ui-sm`（见 `ConfirmDialog`）。
