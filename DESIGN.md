...
- **历史**：左栏提交列表（`git_log` 分页 + `react-window` 虚拟列表，接近底部预取）；中间 Diff 主区（`git_commit_file_contents` + DiffEditor）；右侧提交详情栏（摘要 + 更改文件列表，`git_commit_files`，默认较宽）。左栏与右侧详情栏均可拖拽调宽。不做回退 / cherry-pick。
