...
顶栏窗口控制左侧提供**面板布局**按钮（点击弹出菜单，见 `src/lib/panelLayoutMode.ts`）。快捷键三选一循环并 Toast：
...
双终端 A|B 与编辑器|终端带均可拖；任一侧最大约 **90%**（双终端比例 `0.1–0.9`，终端宽度同带内 10%–90%）。默认比例：编辑器|单终端 **1:1**；编辑器+双终端 **E:A:B ≈ 1:1:1**（编辑器:终端带 = 1:2，双终端内 50%）。顶栏布局菜单只提供三档预设；侧栏布局下，布局按钮左侧另有「双终端」「四终端」「编辑器」独立图标细调（不进菜单；藏编辑器时菜单无勾选项，便于再点预设恢复）。「双」与「四」互斥：四为 2×2（TL/TR/BL/BR），十字分割比例持久化于 `qingcode:quad-terminal-ratio`（列/行各 `0.1–0.9`）。终端栏仅保留收起 / 关闭全部。所有终端标签栏共用溢出下拉。打开文件 / SCM / 设置只显示编辑器列，**不关闭双终端 / 四终端**。
...
**编辑器+双终端** — 编辑器列 + 左右独立「终端」标签栏（溢出下拉）：

```
┌ TitleBar ……………………………… [布局] [─ □ ×] ──┐
├ Act │ Sidebar │ Editor │ TermA │ TermB        │
│ Bar │ Panel   │ (略宽) │ tabs  │ tabs         │
├ StatusBar ────────────────────────────────────┤
```

**四终端（侧栏细调）** — 2×2 四格，每格独立标签栏；可与编辑器列并存，或隐藏编辑器铺满终端带：

```
┌ TitleBar …… [双][四][编辑器][布局] [─ □ ×] ──┐
├ Act │ Sidebar │ Editor（可选）│ TL │ TR       │
│ Bar │ Panel   │               │----+----      │
│     │         │               │ BL │ BR       │
├ StatusBar ────────────────────────────────────┤
```
...
| 标签栏 | 32px | 文件 Tab |
| 终端（底） | 120px – 90% 窗口高 | 可拖拽高度，见 `src/lib/panelLayout.ts` |
| 终端（侧） | 200px – 约 90% 窗口宽（受编辑区最小值约束） | 可拖拽宽度；与高度分别持久化于 `qingcode:terminal-panel`；编辑器可隐藏时终端铺满 |
| 状态栏 | 24px | 项目/文件/终端信息 |
...
| Chip | 文件夹图标 + 项目名（`max-w-[140px]` 截断）+ hover `×` 从顶栏隐藏；右键关闭组为「从顶栏隐藏 / 关闭其它 / 关闭左侧 / 关闭右侧」（不可用项为「移除项目」）；当前项 `bg-bg-active`；不可用项显示 `⚠` + 常驻「重新定位」「移除项目」，点击 chip 体不切换 |
...
当前项目可在项目头与文件树之间显示可折叠的**收藏夹**（无收藏时隐藏）。文件与目录共用一份项目内排序列表：资源管理器右键「收藏文件 / 收藏文件夹」，文件单击直接打开，目录单击在原文件树中展开并定位；收藏项右键复用定位、终端、搜索、复制路径和文件管理器操作。收藏项可拖拽排序，源项半透明，目标上下边缘显示 `accent` 插入线。

启动加载期间不插入仅用于提示的空收藏夹区块，避免无收藏项目的文件树上下跳动；根目录首次扫描完成前显示「正在加载文件树…」，不得将“尚未加载”误报为“空文件夹”。

收藏记录属于本机个人导航状态，保存在 SQLite `favorite_items`，使用项目相对路径，不写入 `.qingcode/project-settings.json`。应用内重命名、移动和永久删除会同步更新收藏路径；Git 分支切换等外部变化造成路径暂时不存在时保留记录并灰显，刷新资源管理器或点击失效项会重新探测。项目根目录不允许收藏（项目切换与排序由标题栏项目 Chip / 多项目工作区负责）。首版不提供分组、自定义别名、代码行书签或团队共享。
...
- **范围**：标题行右侧紧凑下拉（当前项目 / 全部项目）；目录限定芯片仍显示在模式切换下方。
- **触发策略**：文件名模式保留输入后短防抖实时搜索；「全部 / 内容」使用显式提交，按 `Enter` 或点击输入框右侧「查询」执行。修改未提交关键词时保留上一轮结果并显示提示；切换范围、文件类型、大小写等筛选时，使用最近一次已提交关键词自动重查。清空输入同时清空已提交查询和结果。
...
列表 reorder、文件树 pointer DnD、顶栏 chip 排序等**自定义拖拽**统一遵守：
...
资源管理器「打开方式」、`QingCode.exe <file>` 与在线 `open` 命令默认进入同一进程内的独立文件窗口，不切换或污染已有项目窗口；应用内「打开文件」/`Ctrl+O` 仍在当前窗口打开。

- 独立窗口不恢复项目或持久化工作区会话，默认收起侧边栏，可连续承载多个外部文件标签。
- 标题栏以文件图标 +「独立文件」替代项目 chips；面包屑展示完整磁盘路径。
- 状态栏「独立文件」菜单提供在文件管理器中显示、打开所在文件夹、将所在文件夹作为项目打开。
- 用户将文件夹作为项目打开后，该窗口退出独立文件接收角色；后续系统打开请求会创建新的独立文件窗口。
- 单实例转发必须先将目标写入原生的按窗口队列，再通知 WebView，避免窗口加载期间丢失请求。
- Markdown 预览中的本地相对图片以 `.md` 文件所在目录为基准解析；项目外图片按文件精确授权后通过 Tauri asset 协议加载，不扩大为整目录授权。

### 终端标签（TerminalTabs）

文件：`src/components/TerminalTabs.tsx`

经典底部终端、侧栏单终端与双终端两侧共用同一组件，溢出策略与编辑器标签一致（`pickVisibleTabIndices`）。
...
| 溢出 | 有会话时栏右侧常驻 `ChevronDown`；超出宽度时折叠并显示数量角标；下拉列出全部终端，当前项名称后标 `●` |
| 面板操作 | 最右侧 `border-l` 分区：`PanelBottomClose` 收起面板（与溢出下箭头区分）+ `×` 关闭全部 |
...
| 右键「复制文件到剪贴板」 | 将选中文件/文件夹写入系统文件剪贴板（可粘贴到资源管理器 / 支持文件粘贴的聊天软件）；与应用内「复制」独立 | 同左 |
