...
双终端 A|B 与终端带|编辑器均可拖；任一侧最大约 **90%**（双终端比例 `0.1–0.9`，终端宽度同带内 10%–90%）。默认比例：单终端|编辑器 **1:1**；双终端+编辑器 **A:B:E ≈ 1:1:1**（终端带:编辑器 = 2:1，双终端内 50%）。终端栏仍可单独开关双终端/隐藏编辑器。所有终端标签栏共用溢出下拉。打开文件 / SCM / 设置只显示编辑器列，**不关闭双终端**。
