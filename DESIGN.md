...
双终端 A|B 与终端带|编辑器均可拖；任一侧最大约 **90%**（双终端比例 `0.1–0.9`，终端宽度同带内 10%–90%）。默认比例：单终端|编辑器 **1:1**；双终端+编辑器 **A:B:E ≈ 1:1:1**（终端带:编辑器 = 2:1，双终端内 50%）。顶栏布局菜单只提供三档预设；侧栏布局下，布局按钮左侧另有「双终端」「田字终端」「编辑器」独立图标细调（不进菜单；藏编辑器时菜单无勾选项，便于再点预设恢复）。「双」与「田」互斥：田为 2×2（TL/TR/BL/BR），十字分割比例持久化于 `qingcode:quad-terminal-ratio`（列/行各 `0.1–0.9`）。终端栏仅保留收起 / 关闭全部。所有终端标签栏共用溢出下拉。打开文件 / SCM / 设置只显示编辑器列，**不关闭双终端 / 田字终端**。
...
**双终端+编辑器** — 左右独立「终端」标签栏（溢出下拉）+ 编辑器列：

```
┌ TitleBar ……………………………… [布局] [─ □ ×] ──┐
├ Act │ Sidebar │ TermA │ TermB │ Editor        │
│ Bar │ Panel   │ tabs  │ tabs  │ (略宽)        │
├ StatusBar ────────────────────────────────────┤
```

**田字终端（侧栏细调）** — 2×2 四格，每格独立标签栏；可与编辑器列并存，或隐藏编辑器铺满终端带：

```
┌ TitleBar …… [双][田][编辑器][布局] [─ □ ×] ──┐
├ Act │ Sidebar │ TL │ TR │ Editor（可选）      │
│ Bar │ Panel   │----+----│                     │
│     │         │ BL │ BR │                     │
├ StatusBar ────────────────────────────────────┤
```
...
| 项 | 约定 |
|----|------|
| 布局 | 标题栏左侧：应用图标 + 常驻工作区菜单 + 项目 chips 区（`flex-1`）+ 拖拽 spacer（固定 `w-[140px]`）+ QingCode；右侧：侧栏布局时双终端/田字终端/编辑器快捷开关 + 面板布局切换 + 窗口按钮 |
...
| 溢出 | 有会话时标签区右侧常驻 `ChevronDown`；超出宽度时折叠并显示数量角标；下拉列出全部终端，当前项名称后标 `●` |
...
- **刷新 / 文件监视**：工具栏刷新与外部变更自动刷新会对**已展开目录重新扫描磁盘**（`reloadLoadedChildren`），不只更新根目录一层。监视默认开启（Tauri）；`explorer.excludeGitIgnore` 默认 false（gitignore 条目仍显示；设为 true 可隐藏）。
