...
- **青台** 上下两层**等高**（各 40px，底边距画布约 34px）：上层 `#4ECDB5`、下层 `#2FAF9B`，底边直角长方形
- **厚切括号** `#F3F6F8`，居中留白，约占画布高度 40%
- **桌面打包**（`app-icon-file.svg`）使用黑色圆角底 + 轻微径向渐变，与 ZCode / OpenCode 等桌面图标风格对齐

修改 `src/assets/app-icon.svg`（并同步 `public/app-icon.svg`）后，UI / Splash 会跟随；打包 exe 前需同步 Tauri 位图图标：
