...
文件：`src/components/SourceControlPanel.tsx`

- Git 状态保留 porcelain `XY` 双列语义，同一文件可同时出现在“已暂存的更改”和“更改”分组；资源树、标签和角标仍显示单个友好状态字母。
- “更改”和“已暂存的更改”使用上下两个独立虚拟列表，分组标题固定且可折叠；提交信息、提交后推送开关和提交按钮固定在面板底部。
- 面板支持单个/全部暂存、单个/全部取消暂存、只提交已暂存内容，以及向当前分支已配置的 upstream 执行普通 `git push` 与 `git pull`；写操作完成后统一执行完整状态刷新。
- 提交信息通过系统 Git stdin 传递，正常执行用户身份检查和 hooks；失败时保留输入并显示 Git 输出。
- 拉取后若存在未合并路径（`UU` 等），面板顶部显示冲突提示横幅，并在 Toast 中说明；不提供冲突解决器，需在编辑器中手动处理标记后暂存提交。
- 不提供隐式暂存、discard、fetch、远程凭据管理或内置冲突合并 UI。详细安全边界、异常处理和验收场景见 [`docs/git-basic-commit-workflow.md`](docs/git-basic-commit-workflow.md)。
