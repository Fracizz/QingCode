...
编辑器内置不依赖完整 LSP 的轻量代码导航：按住 `Ctrl` 时标记可跳转符号，`Ctrl+单击` 解析并跳到定义；`Shift+F12` 在可拖动的半透明浮层中按调用者或文件分组查找调用、读取、写入和导入位置，`Ctrl+T` 按名称搜索工作区符号。Tree-sitter 增量索引会识别常见作用域、遮蔽和项目内模块关系，包括 TS/JS 路径别名与 re-export、Python 包转发、Java import、Rust `use`、Go module/import；无法精确判定的结果会明确标为近似。
