...
源代码管理显示当前分支、已暂存 / 未暂存的修改和差异，可与 HEAD 并排比较；支持暂存、取消暂存、拉取和提交，提交时可选择一并推送到远程。中文、空格、重命名路径按原始路径读取。复杂的分支整理、rebase 等操作仍可在你熟悉的 Git / AI 工具里完成。

### 搜索与定制

按范围搜文件名或文件内容。深色 / 浅色 / 森林 / 跟随系统；界面与代码字体可调；界面语言支持简体中文与 English。全局 `default-settings.json` 与项目 `.qingcode/project-settings.json` 为 **JSON5**，模板注释标明不得删除注释（详见 [帮助文档 · 设置](./帮助文档.md#设置)）。
