...
- 支持工作目录与环境变量；可停止整组配置  
- 每个配置可决定是否跟随项目会话恢复，默认关闭；开启后重启应用会恢复并重启其关联终端
...
标题栏提供**面板布局**菜单（快捷键可循环），默认是**经典布局**（终端在底部）。也可切到侧栏旁工作区：

| 布局 | 说明 |
|------|------|
| 经典布局（终端在底部） | 编辑器为主，终端可拖高度 |
| 终端+编辑器 | 侧栏旁单终端 \| 编辑器，默认可拖成约 1:1 |
| 双终端+编辑器 | 终端 A \| 终端 B \| 编辑器，默认约 1:1:1；中间与右侧分隔都可拖（约 10%–90%） |

双终端两侧各自有标签栏，标签过多时用下拉查看全部。侧栏布局下可在顶栏布局按钮左侧开关双终端或隐藏编辑器；打开文件 / 源代码管理 / 设置时会显示编辑器列，但不会关掉双终端。
...
资源管理器只展示当前项目。可在树里新建、重命名、删除；复制路径、相对路径、文件引用，或「复制文件到剪贴板」粘到资源管理器 / 支持文件粘贴的应用；也可在指定目录打开终端或搜索。默认显示项目中的全部条目，包括构建产物、依赖目录和被 `.gitignore` 忽略的内容；可用 `files.exclude` 或 `explorer.excludeGitIgnore` 按需隐藏。磁盘变更会自动刷新已展开目录，工具栏刷新同样会重扫展开子树。

多文件标签编辑，常见语言按需高亮；自动识别 UTF-8 / BOM / GB18030 兼容编码；外部改动可分类处理，避免悄悄覆盖。大文件会降级或只读预览。重启后恢复各项目的编辑与终端会话（打开项目时不再自动新建空终端）。

编辑器内置不依赖完整 LSP 的轻量代码导航：按住 `Ctrl` 时标记可跳转符号，`Ctrl+单击` 解析并跳到定义，落点稳定后自动执行一次查找用法；`Shift+F12` 也可独立在可拖动的半透明浮层中按调用者或文件分组查找调用、读取、写入和导入位置，`Ctrl+T` 按名称搜索工作区符号。Tree-sitter 增量索引会识别常见作用域、遮蔽和项目内模块关系，包括 TS/JS 路径别名与 re-export、Python 包转发、Java import、Rust `use`、Go module/import；无法精确判定的结果会明确标为近似。

它适合日常读代码、跨文件跟调用和快速定位；和 IDEA 相比，不做编译器级类型分析、安全重命名 / 重构或调试，复杂语言语义仍建议回到 IDEA 等完整 IDE。Python、TypeScript/JavaScript、Java、Rust、Go 拆成五个独立语言组件；Windows 安装包可逐项勾选，未安装的语言只会关闭该语言的语义导航，不影响普通文本编辑。
...
| 日常场景 | QingCode | IDEA | VS Code | Zed |
|--|--|--|--|--|
| 最适合做什么 | 同时管多个项目、服务和终端现场 | 深度开发、重构、调试 | 按扩展拼装的通用开发 | 轻快编辑与 LSP 开发 |
| 切项目后 | 标题栏常驻，编辑与终端现场保留 | 以项目 / 窗口为中心 | 以工作区为中心 | 以工作区为中心 |
| 代码导航 | Tree-sitter：定义、引用、符号；有近似结果提示 | 编译器级语义、重构与调试 | 主要随语言服务器和扩展而定 | 主要随语言服务器而定 |
| 扩展与 AI | 无市场、不内置模型；终端 + CLI 对接外部助手 | 插件生态与 AI 插件 | 扩展生态最广 | 内置 AI 倾向、生态较小 |

常见搭配：先在 QingCode 一键启动 API、前端和 Worker，切项目时保留现场；需要类型推断、重构或断点调试时，再用 IDEA / VS Code / Zed 打开同一仓库。QingCode **刻意不做**完整 IntelliSense、调试器和插件市场。

## 典型用法

1. 添加几个本地项目目录，钉在标题栏  
2. 为常用项目写好运行配置（例如 `dev` = API + Web）  
3. 需要时一键启动服务，终端按任务分开；可用「双终端+编辑器」并排盯两路输出  
...
- Windows：便携 exe 或 NSIS 安装包（`*-setup.exe`）；需 [WebView2](https://developer.microsoft.com/microsoft-edge/webview2/)（较新系统通常已预装）。安装包会先尝试自动下载安装；失败时可点「是」跳转引导程序下载，或「否」打开产品页。轻量代码导航语言组件当前由安装包按需安装：五项默认全选，可按需取消；便携单文件不内置这些组件
