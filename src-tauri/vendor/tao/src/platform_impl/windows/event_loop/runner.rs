...
impl<T> EventLoopRunner<T> {
  ...
  pub(crate) fn reset_runner(&self) {
    ...
    // Take the boxed callback out of the Cell, then leak it.
    //
    // `event_handler.set(None)` drops a ~0x260-byte `Box<dyn FnMut>` (Tauri's
    // run closure). That Drop re-enters Win32 window/WebView teardown and has
    // been observed as HEAP_FAILURE_BLOCK_NOT_BUSY / double-free on Windows
    // (`RtlFreeHeap` of the same block during this function's epilogue).
    // `EventLoop::run` calls `process::exit` immediately after `run_return`,
    // so leaking until process exit is equivalent to skipping a destructor
    // the OS is about to reclaim anyway.
    mem::forget(event_handler.take());
  }
}
