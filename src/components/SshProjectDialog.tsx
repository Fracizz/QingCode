...
export default function SshProjectDialog({ open: visible, onClose, onAdded }: Props) {
  ...
  const pathFieldClass = `${fieldClass} font-mono text-ui-sm`
  const labelClass = 'text-ui-sm text-fg-muted'
  const helpClass = 'text-ui-sm leading-5 text-fg-dim'
  ...
  return (
    <ModalOverlay onDismiss={handleClose} zIndex="z-[120]">
      <div
        ...
        className={`ui-font-scaled modal-content-enter relative flex w-full max-w-[480px] shrink-0 flex-col overflow-hidden rounded-lg border border-border-strong bg-bg-elevated shadow-2xl shadow-black/50 ${
          step === 'browse' ? 'h-[520px] max-h-[80vh]' : 'max-h-[80vh]'
        }`}
        ...
        {step === 'connect' ? (
          <>
            <div className="grid min-h-0 flex-1 grid-cols-2 gap-x-3 gap-y-3 overflow-y-auto px-4 py-4">
              ...
              {creatingConnection ? (
                ...
                <>
                  <div className={`col-span-2 ${labelClass}`}>
                    已保存的主机
                    <div className="mt-1 flex items-center gap-2 rounded border border-border-strong bg-bg px-2.5 py-1.5 font-mono text-ui-sm text-fg-muted">
              ...
              {error ? (
                <p className="col-span-2 rounded border border-danger/30 bg-danger/10 px-2.5 py-2 text-ui-sm text-danger whitespace-pre-wrap">
                  {error}
                </p>
              ) : null}
            </div>
          ...
          <>
            <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden px-4 py-4">
              ...
              <div className={labelClass}>
                远程主机
                <div className="mt-1 flex items-center gap-1.5 rounded border border-border-strong bg-bg px-2.5 py-1.5 font-mono text-ui-sm text-fg-muted">
                ...
                <div className="flex gap-1.5">
                  <Tooltip label="上级目录" side="bottom">
                    <button
                      ...
                      className="inline-flex h-[30px] w-[30px] flex-shrink-0 items-center justify-center rounded border border-border-strong text-fg hover:bg-bg-hover disabled:opacity-40"
                    >
                      <ArrowUp size={14} />
                    </button>
                  </Tooltip>
                  <input
                    ...
                    className={`${pathFieldClass} min-w-0 flex-1 py-1`}
                  />
                </div>
                <div
                  role="listbox"
                  aria-label="远程文件夹"
                  className="min-h-[200px] flex-1 overflow-y-auto rounded border border-border-strong bg-bg"
                >
                  {browsing ? (
                    <p className="px-2.5 py-4 text-center text-ui-sm text-fg-muted">
                      正在读取目录…
                    </p>
                  ) : entries.length === 0 ? (
                    <p className="px-2.5 py-4 text-center text-ui-sm text-fg-muted">
              ...
              {error ? (
                <p className="rounded border border-danger/30 bg-danger/10 px-2.5 py-2 text-ui-sm text-danger whitespace-pre-wrap">
                  {error}
                </p>
              ) : null}
            </div>
